...
export interface MethodPolicyCodeCatalogItem {
  code: number;
  title: string;
  description: string;
...
export interface MethodPolicyCodeCatalogResponse {
  data: MethodPolicyCodeCatalogItem[];
  meta: {
    count: number;
  };
}

export function fetchMethodPolicyCodeCatalog(): Promise<MethodPolicyCodeCatalogResponse> {
  return apiGet<MethodPolicyCodeCatalogResponse>(
    '/method-settings/codes',
...
export interface SaveMethodPolicyOverrideRequest {
  http_method: HttpMethod;
  status_code: number;
  company_id?: string | null;
  project_id?: string | null;
...
export function saveMethodPolicyOverride(
  ...
  body: SaveMethodPolicyOverrideRequest,
): Promise<{ data: Record<string, unknown> }> {
  return apiPatch(`/method-settings/${scope}`, { apiVersion: 'v1' }, body);
}
