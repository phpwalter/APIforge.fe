...
import { WorkspaceFromVersionControlModal } from './WorkspaceFromVersionControlModal';
...
describe('WorkspaceFromVersionControlModal', () => {
  ...
  it('walks repo → branch → file and imports the selected file, closing the modal', async () => {
    ...
    useAppStore.setState({ authProvider: 'github', workspaceFromVersionControlOpen: true });
    ...
    await waitFor(() => expect(useAppStore.getState().workspaceFromVersionControlOpen).toBe(false));
  });
});
